...
uint8_t imuRead(uint8_t reg){
    ...
    uint8_t bytes[3] = {IMU_READ | reg, 0x00, 0x00};
...
void imuWrite(uint8_t reg, uint8_t data){
    ...
    uint8_t bytes[2] = {IMU_WRITE | reg, data};
 ...
 * 
 * @details Per bmi270 datasheet: After every POR or soft reset, the IMU remains in suspend mode. To get ready for operation the device must be initialized through the following procedure: 
...
void imuInit(void)
{
    ...
    // 3. prepare config load
    imuWrite(0x59, 0x00);

    // 4. burst write to reg INIT_DATA 
    uploadConfigFile();

    // 5. complete config load
    imuWrite(0x59, 0x01);
