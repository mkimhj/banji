...
void bmi2_print_error_code(int8_t rslt);
...
int8_t imuWrite(uint8_t reg_addr, const uint8_t *reg_data, uint32_t len, void *intf_ptr){
    ...
    /*
    if(len == 1){
        NRF_LOG_RAW_INFO("len = %d (raw) reg_addr = %x, data = %x\n",len,  reg_addr, *reg_data);
    }
    else if(len == 2){
        NRF_LOG_RAW_INFO("len = %d (raw) reg_addr = %x, data = %x %x \n", len,  reg_addr, *reg_data, *(reg_data + 1));
    }
    else{
        NRF_LOG_RAW_INFO("len = %d (raw) reg_addr = %x, data = %x %x %x \n", len,  reg_addr, *reg_data, *(reg_data + 1), *(reg_data + 2));
    }
    */
